...
bool gic_validate_vgic(void)
{
	...
	/* Read and validate gic LRs as set by Host */
	read_lrs(&gicstate);

	return gic_validate_lrs(gicstate.ich_lr_el2);
}
...
bool gic_is_any_interrupt_pending(void)
{
	...
	/* Read and validate ICH_LR_EL2 set by Host */
	read_lrs(&gicstate);
...
void gic_disable_virtual_cpuif(void)
{
	...
	write_ich_hcr_el2(read_ich_hcr_el2() & ~ICH_HCR_EL2_EN_BIT);
}
...
void gic_init_vgic_state(struct gic_cpu_state *gicstate,
			...
			unsigned long gicv3_hcr)
{
	gic_copy_state_from_plane_entry(gicstate, gicv3_lrs, gicv3_hcr);
