 ...
 * granules store the regular RMM stack and the exception-handler stack.
 *
...
/* Empty virtual page between the metadata page and the regular stack. */
#define PCPU_METADATA_GAP_VIRTUAL_PAGE	U(1)
/* Empty virtual page between the regular stack and the EH stack. */
#define STACK_GAP_VIRTUAL_PAGE		U(1)
/* Empty virtual page between the EH stack and the slot-buffer range. */
#define EH_STACK_GAP_VIRTUAL_PAGE	U(1)
/*
 ...
 */
#define PCPU_REGION_TOTAL_VA_PAGES	(PCPU_REGION_PAGES + STACK_GAP_VIRTUAL_PAGE + \
					PCPU_METADATA_GAP_VIRTUAL_PAGE + EH_STACK_GAP_VIRTUAL_PAGE)

/* Virtual address boundaries of the fixed per-CPU high-VA layout. */
#define PCPU_METADATA_BASE_VA	(SLOT_BUFFER_BASE_VA - (PCPU_REGION_TOTAL_VA_PAGES * GRANULE_SIZE))
#define PCPU_METADATA_TOP_VA	(PCPU_METADATA_BASE_VA + (PCPU_METADATA_PAGES * GRANULE_SIZE))
#define STACK_BASE_VA		(PCPU_METADATA_TOP_VA + (PCPU_METADATA_GAP_VIRTUAL_PAGE * GRANULE_SIZE))
#define STACK_TOP_VA		(STACK_BASE_VA + (RMM_NUM_PAGES_PER_STACK * GRANULE_SIZE))
#define EH_STACK_BASE_VA	(STACK_TOP_VA + (STACK_GAP_VIRTUAL_PAGE * GRANULE_SIZE))
#define EH_STACK_TOP_VA		(EH_STACK_BASE_VA + (EH_STACK_PAGES * GRANULE_SIZE))
 ...
 * the region itself, the regular RMM stack, and the exception-handler stack.
 *
...
struct pcpu_metadata {
	 ...
	 * This is also equal to the physical address this struct.
	 */
