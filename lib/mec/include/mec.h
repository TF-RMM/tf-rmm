...
/* Shared MECID comes after the reserved MECIDs */
#define MECID_SHARED	RESERVED_IDS

#define MECID_PRIVATE_START	(MECID_SHARED + 1U)
...
struct mec_state_s {
	/*
	 * The mec_reserved array defines the state of all MECs in
	 * the system.
	 *
	 * For a given mecid:
	 *
	...
	/* Spinlock to protect access to shared MECID members count */
...
static inline unsigned int mecid_private_count(void)
{
	...
	if (max_mecid < MECID_PRIVATE_START) {
		return 0U;
	}

	return max_mecid - MECID_PRIVATE_START + 1U;
}
...
static inline void mec_realm_mecid_s2_init(unsigned int mecid)
{
	...
	if (is_feat_mec_present()) {
		...
		write_vmecid_p_el2((unsigned long)mecid);
